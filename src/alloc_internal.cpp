...
AllocInternal *gAllocInternal =
    new AllocInternal(); // let it leak, because rust will call dealloc after
                         // cpp.exit(msvc)
...
void alloc_enum(void *usr_data, int (*cb)(void *usr_data, size_t address,
                                          size_t size, const size_t *stack)) {
